...
def predict(req: PredictRequest):
    ...
    # у тебя валидация сейчас показывает "onnx_output=probability (no sigmoid)" — значит уже probability.
    prob = float(np.array(y).reshape(-1)[0])

    threshold = 0.5
    pred_class = int(prob >= threshold)

    return {"class": pred_class, "probability": prob, "threshold": threshold}
